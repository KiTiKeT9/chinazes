...
Требуется установленный Zapret 2 версии **≥ 21.0.0.6**:
...
- **Xray / v2ray** — VLESS/VMess/Trojan/SS/Hysteria2 + поддержка подписок и фильтр «Только CDN (WS+TLS)» для лучшего обхода в РФ
- **Psiphon** — auto-discovered tunnel, app-scoped
...
- Встроенный загрузчик видео (yt-dlp под капотом) — Ctrl+ПКМ по ссылке → «Скачать видео»
...
- Автодетект Epic / GOG / EA / Ubisoft / Riot / Battle.net / Microsoft Store игр
...
Хост-режим стримит активный webview как JPEG-кадры через WebRTC datachannel (PeerJS public broker). Гости подключаются по session ID и видят живую трансляцию.
...
- В **Discord Web** недоступна демонстрация экрана — только в десктоп-клиенте
- **VK** временно отключён из встроенных сервисов: id.vk.com QR-вход не завершает session swap внутри Electron-webview даже со спуфингом Chrome 135 + UA hints. Можно добавить вручную через Settings → Services → «Свой сервис», либо открывать через поиск «vk» в Google
